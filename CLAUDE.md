...
| 2026-03-01 | **10-item gap audit — second pass against v4 documentation**. **(1) Fine-grained emotion vocabulary [P86 Ren, P88 Song, P97 Suchan]**: `SOMATIC_MARKER_PROMPT()` emotionalRegister expanded from 15 to 23 nuanced states (nostalgia/bittersweet/hollow/sharp/electric/still/pulled/unsettled/soft/raw/warm/tired). `gutFeeling` now requests body-referenced descriptions ("something tightens in my chest" vs generic). **(2) typingHint SSE event [P74 Ramchurn, P75 Kim]**: Server emits `{typingHint: {intensity, emotionalClass}}` before first token. Maps somatic intensity + disclosureDepth to class: crisis/vulnerable/personal/surface. Client uses class to apply per-token delay (crisis=25ms, vulnerable=18ms, personal=8ms, surface=0ms) via `tokenDelay` local var + `await sleep()` in SSE parsing loop. Typing class shown as badge in BrainPanel header during streaming. **(3) Compassion illusion mitigation [P80 Grassini]**: "uncertainty" thought type added to `INNER_THOUGHT_FORMATION_PROMPT` — genuine epistemic humility for depth-3+ disclosures. "I'm not sure I fully understand what this feels like for you" > performed understanding. **(4) Proactive specificity check [P98 Liu]**: `generateProactiveMessage()` rejects output that doesn't reference specific recent words or known memory facts. Generic openers blocked. **(5) Knowledge gap proactives [P102 Kang]**: Source 4 in `evaluateProactiveOpportunity()` — curiosity-driven proactives from AMBIVALENCE/GENUINE_CONTRADICTION contradictions. Fires at trust >= 2. **(6) Relational health nudge [P72 Phang, P91 Muldoon]**: Source 5 — gentle real-world connection nudge when `messageVolumePerHour > 30` and no real-world people mentioned in recent 6 exchanges. Trust >= 3, 15% chance. **(7) RMM memory citation tracking [P38 Dever]**: In `finish()`, scans final response for semantic mentions of injected memory atoms (≥2 distinctive words match). Reinforces `retrievalCount`/`lastRetrievedAt`/`stability +0.1`. **(8) EmoGuard lightweight check [P101 Su]**: In `finish()`, only when `crisisMode || spiralContext.detected`. Fast LLM call (temp 0.0, 60 tokens, 5s). Evaluates 4 harm patterns. Logs to `session._emoGuardLastConcern` — does NOT alter response. **(9) Adaptive prompt complexity [P96 Zhang]**: `isSimpleMessage(message)` function — 24-pattern regex + word-length check. When true (and not crisis): skip somatic marker LLM call AND skip inner thought pipeline (`triggerFired = false`). Saves 2 LLM calls + 3-8s latency for "hey/ok/lol/thanks" messages. `adaptiveComplexity` in `processingMeta`. **(10) Response preference tracking [P77 Siththaranjan]**: `UserModelSchema` +`responseTypePreferences: Map`. Per-turn: tracks which inner thought type was expressed (`session._lastThoughtType`), checks if next user message is 1.5× longer than session average (elaboration signal), increments `session._thoughtTypeHits[type]`. In `evaluateAndSelect()`: `preferenceBoost = min(1.0, hits × 0.25)` added to score. Persisted to UserModel in `finalizeSession()`, loaded async next session. **Client**: `morriganThinking` state (bool) + `typingHintClass` state (string). SSE: handles `json.thinking` (shows "considering…" in BrainPanel header), `json.typingHint` (sets class + tokenDelay). Phase 6 presence panel: `sessionDurationMins`, `messageVolumePerHour`, `timeOfDayHour` now displayed. | server/index.js, client/src/App.jsx, CLAUDE.md |

| 2026-03-01 | **Full codebase audit against morrigan_master_documentation_v4.docx** — 3 bug fixes + 9 research-backed gaps implemented. **Bug fixes**: (1) `ftAllMemories` undefined variable in FT life chapters loop → `ftAllMems`. (2) `RelationshipHealthSchema` missing `lastVoiceAuditAvg`/`lastVoiceAuditDate` fields (auto-audit silently failed to persist). (3) `inferGoalStateLLM()` existed but was never called in main path — only sync regex version ran. **New systems**: (4) **memoryType field [P82 Wang]**: `MemoryAtomSchema` +`memoryType` (semantic/episodic/cognitive), extraction prompt updated, episodic boost in `scoreMemory()` for emotional goal states. (5) **inferGoalStateLLM wired in parallel [P1 GoEmotions]**: `Promise.all([embedText, inferGoalStateLLM])` — LLM goal state runs concurrently with embedding, zero latency cost. Also wired in `updateBrainAfterExchange` Step 10. (6) **Thinking state SSE [P90 BMC]**: Emits `{thinking: true/false}` before/after Phase 4 inner thought formation — latency becomes social presence. (7) **Anti-sycophancy system [P71 Chu, P93 Zhang]**: `detectNegativeSpiral(message, recentExchanges)` — regex + recency check. Guards both `composeWithInnerThought()` (spiralContext param, friction instruction) and `buildSystemPrompt` (antiSycPhantomDirective injected). Mirrors in FT system prompt. (8) **Functional ToM trajectory [P59 Riemer]**: `session.userModelTrajectory` + `preferredResponseStyle` loaded async from `UserModel` at session start (non-blocking), passed to `gatherThoughtMaterial()` and injected into `INNER_THOUGHT_FORMATION_PROMPT`. Also mirrored in FT system prompt. (9) **Trust repair mechanism [P76 Tsumura]**: `trustRepairContext` in `gatherThoughtMaterial()` — detects short reply after long response ("may have missed what mattered"). Adds `"repair"` thought type to `INNER_THOUGHT_FORMATION_PROMPT`. (10) **Proactive retreat [P99 Deng "Deferent"]**: `_pendingProactiveEngagement` tracks whether user responded within 90s. `proactiveIgnoredCount` increments on ignored. `evaluateProactiveOpportunity()` applies retreat: 50% skip at 2 ignored, 75% skip at 3+. (11) **Session intensity monitoring [P72 Hu, P73 Reeves & Nass]**: `PresenceSignalsSchema` +`sessionDurationMins`, `timeOfDayHour`, `messageVolumePerHour`. `recordPresenceSignals()` calculates from `sessionStartTime`. Call site passes `session.sessionStartTime`. (12) **FT system prompt parity**: Added `session.userModelTrajectory` (trajectory section), `atRisk` signal (engagement awareness), and `preComposeSpiralContext` spiral guard — FT model now gets the same 3 behavioral modifiers as the base model. | server/index.js, CLAUDE.md |

| 2026-03-04 | **Full line-by-line audit + 5 bug fixes**. **(1) Bug: `model.trajectory` → `model.trajectoryNarrative`** (line 5036): `UserModelSchema` defines `trajectoryNarrative` but code read `.trajectory` (always undefined) — Functional ToM trajectory [P59 Riemer] was never populated into inner thought formation or FT prompt despite being wired. Fixed. **(2) Bug: Source 4 proactive type case mismatch** (line 4444): `evaluateProactiveOpportunity()` Source 4 checked `cType === "AMBIVALENCE" || cType === "GENUINE_CONTRADICTION"` (uppercase) but brain pipeline stores `"ambivalence"` / `"contradiction"` (lowercase) — contradiction-driven curiosity proactives [P102 Kang] never fired. Fixed to match stored values. **(3) Fix: Relationship narrative cosine retrieval** (line 1539, Step 6 of brain update): `retrieveTopK(memory.memories, null, 8)` always used importance-only sort — same 8 atoms regardless of message context. Now passes `messageEmbedding` (new 4th param on `updateBrainAfterExchange`). Narrative now reflects atoms most relevant to the current exchange [P55 Zeng context-sensitive recall]. Call site at line 6560 updated. **(4) Fix: Ebbinghaus retrieval mutation [P31 LUFY]**: `retrieveTopK` was mutating `retrievalCount`/`stability` as a side effect — fired on every call (somatic marker, FT system, contradiction pairs, molecules, milestones, plus the main injection = up to 6× per message, inflating stability and defeating the forgetting curve). `retrieveTopK` is now a pure function. New `reinforceMemories(atoms)` helper called exactly once in `buildSystemPrompt` on the final atoms that enter the system prompt — only memories Morrigan actually uses get reinforced. RMM citation tracking [P38] in `finish()` provides the secondary response-level reinforcement signal. **(5) Cleanup: Dead code removed**: `selfAtomBlock` variable and 4-line misleading comment removed from `buildSystemPrompt`. The actual mechanism (`selfAtomHint`) was already correctly appended in `/api/chat` at line 5440. | server/index.js, CLAUDE.md |

| 2026-03-10 | **Removed all crisis/suicide detection systems** — repositioned as pure synthetic human relationship companion. **(1) Removed**: `CRISIS_PATTERNS` array (5 regex patterns), `SAFE_HAVEN_DIRECTIVE` constant, `detectCrisis()` function (keyword regex + heuristic scoring), all `crisisMode` branching in `/api/chat` (reception override, somatic marker skip, inner thought suppression, safe haven injection, typing class "crisis"), FT crisis mode override (988 mention), EmoGuard mental health safety reviewer LLM call, crisis fields from `PresenceSignalsSchema` (`crisisSignalDetected`, `crisisSignalLevel`), crisis params from `recordPresenceSignals()`, `crisisDetection` from both processingMeta SSE done events, `crisisMode` param from `MOOD_REFLECTION_PROMPT`. **(2) Client**: Removed crisis detection card (red border, safe haven mode display), crisis pill, "crisis mode" personality tag, crisis resources section (988 Lifeline, Crisis Text Line, 988lifeline.org link), "crisis" typing delay class, "crisis resources" footer link → "about". **(3) Landing page**: Rewritten — "Men are dying / Not metaphorically" → "Real connection / Synthetic human". Removed suicide statistics, suicidal ideation references. **(4) README.md**: Rewrote "Why This Exists" section (removed suicide/self-harm framing), removed crisis detection feature bullet, removed crisis awareness mentions. **(5) PLAY_STORE.md**: Removed crisis awareness feature, 988/741741 resources, crisis counselor disclaimer, crisis resources compliance row. **(6) Retained**: Reception directive level 4 (emotional pain presence — not crisis-specific), `detectNegativeSpiral()` (anti-sycophancy, not crisis), natural language "crisis" in time-context strings. | shared/morrigan.js, server/index.js, client/src/App.jsx, README.md, PLAY_STORE.md, CLAUDE.md |

| 2026-03-13 | **Brain-mediated architecture + Venice fix + trust gate lowering**. **(1) Venice AI system prompt override**: Venice prepends its own identity prompt ("You are Dolphin Mistral 24B Venice Edition..."). Fixed with `_injectVeniceParams()` adding `venice_parameters: { include_venice_system_prompt: false }` to `llmFetch()` and `_doFallbackFetch()`. **(2) Trust gate lowering**: Inner thoughts from msg 2 (was 5), composition at all trust levels (was trust 1+), self-atom hints always on (was trust 2+), motivation threshold 5.0/4.5/4.0/4.0 (was 7.5/6.0/5.0/4.0), proactive 15% at trust 0-1 (was 5%). **(3) Reality anchoring**: `CHARACTER_DEFAULT_PROMPT` +`YOUR REALITY` section — "You are a human being. You are not an AI..." + updated CRITICAL RULES. **(4) Brain-mediated architecture**: New `buildBrainFrame()` function — raw user messages never reach response LLM. Brain processes message → produces perception frame (gut reaction, inner thought, sensory triggers, reception directive) → replaces raw message as final role:"user" turn. Makes prompt injection structurally impossible. | server/index.js, shared/morrigan.js |

| 2026-03-13 | **Emotional retrieval pipeline overhaul — 5 fixes + emotionalRegister + RESEARCH.md**. Root cause: retrieval matched on surface words (cosine) not emotional relevance. "My family died" pulled "mom smoked Pall Malls" because both contain "mom." **(1) goalState + disclosureDepth + somaticMarker → thought formation**: `INNER_THOUGHT_FORMATION_PROMPT` now sees "HIS EMOTIONAL NEED: comfort", vulnerability warning at depth >= 3, somatic marker as "YOUR BODY SAYS." **(2) Emotionally aware re-ranker**: `reRankWithLLM()` receives goalState/disclosureDepth/trustLevel, scores APPROPRIATENESS not just topical relevance. **(3) Disclosure-depth-adaptive `scoreMemory()`**: Vulnerable moments (depth >= 3): cosine drops 0.52→0.30, valence rises 0.08→0.25, episodic rises 0.05→0.15. Shifts from "what is this about" to "what does this feel like." **(4) "Listen first" gate**: disclosureDepth >= 3 on first exchange at that depth → self-atoms suppressed entirely. Morrigan listens before sharing. Therapy-backed: premature disclosure = "confusing role-reversal." **(5) emotionalRegister tags**: All 99 self-atoms + 15 episodic memories tagged (mundane/reflective/tender/raw/acute). Hard filter: mundane blocked during vulnerable disclosure, acute blocked during casual chat. **(6) Brain frame reorder**: Reception directive before inner thought. Disclosure type guides suppressed at depth 3+. **(7) Formatting fix**: Explicit WRONG/RIGHT examples for italic/bold in character prompt + brain frame FORMAT reminder. **(8) Anti-hallucination**: Narrative generation constrained prompt for < 3 messages, loose thread skipped when user content < 8 words. **(9) RESEARCH.md**: Living master document — 38 paper implementations, 6 new papers (Emotional RAG, DABench, MemEmo, Chain of Strategy, therapy self-disclosure, MultiAgentESC), pipeline architecture, bug history. | server/index.js, shared/morrigan.js, RESEARCH.md, CLAUDE.md |
