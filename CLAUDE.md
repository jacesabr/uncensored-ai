...
COLAB_URL=https://YOUR_MODAL_URL.modal.run    # Base URL for LLM (no /v1)
LLM_API_KEY=...                                # Bearer token for LLM provider
# Embeddings — separate provider since fine-tuned model doesn't serve embeddings
EMBED_URL=https://api.venice.ai/api            # (defaults to COLAB_URL if not set)
EMBED_API_KEY=...                              # (defaults to LLM_API_KEY if not set)
...
| 2026-02-28 | **Morrigan SFT deployment + provider-agnostic LLM integration**. **(1) `deploy_modal.py` rewritten**: ASGI app via `@modal.asgi_app()` exposing `/v1/chat/completions` with SSE streaming + non-streaming, `/v1/models`, `/health`. Uses vLLM synchronous engine with chunked token streaming. Model cached in Modal volume. OpenAI-compatible — server needs zero URL changes. **(2) Server env var generalization**: `VENICE_API_KEY` → `LLM_API_KEY` (with `VENICE_API_KEY` backward compat). New `EMBED_URL` + `EMBED_API_KEY` env vars for separate embedding provider (fine-tuned model doesn't serve embeddings). All 33 `VENICE_API_KEY` refs updated. 4 embedding call sites (`embedText`, self-atom seeding ×2, status ping) now use `EMBED_URL`/`EMBED_API_KEY`. `CHAT_MODEL` default changed to `morrigan-sft-v1`. **(3) `test_endpoint.py`**: API key moved from hardcoded to `RUNPOD_API_KEY` env var. **(4) `DEPLOY_MORRIGAN.md`**: Comprehensive multi-provider deployment guide (Modal, RunPod, TGI, Ollama) with cost comparison and integration examples. | server/index.js, deploy_modal.py, deploy_runpod.py, test_endpoint.py, DEPLOY_MORRIGAN.md, CLAUDE.md |
